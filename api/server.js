...
async function initializeDataMode() {
    // Check if we can connect to Mongo; otherwise fall back to static files.
    try {
...
if (!isVercel) {
    ...
    initializeDataMode().catch(err => {
        console.warn("\x1b[33m⚠️ Engine initialization warning:\x1b[0m", err.message);
        dataManager.setStaticMode(true);
    });
}
