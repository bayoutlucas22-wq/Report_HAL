...
function precomputeANP() {
  ...
  const csvPath = locate("src/data/incidentes.csv") || locate("data/incidentes.csv");
  ...
  const ANP_RECORDS = lines.slice(1).map(line => {
    ...
    return {
      ...
      descricao: get(16), codigo: get(17), tipos, category
    };
  }).filter(Boolean);
  ...
  const yearSeries = Object.entries(yearMapObj).sort((a,b) => a[0].localeCompare(b[0])).map(([year, count]) => ({ year, count }));
  const halYearSeries = Object.entries(halYearMap).sort((a,b) => a[0].localeCompare(b[0])).map(([year, count]) => ({ year, count }));
  const topCompanies = Object.entries(companyMap).sort((a,b) => b[1]-a[1]).slice(0,10).map(([name, count]) => ({ name, count }));

  const ANP_STATS = { total: ANP_RECORDS.length, fatalidades: totalFatal, feridos: totalInj, csbCount, bopCount, kickCount, structCount, yearSeries, halYearSeries, categoryMap, topCompanies };
...
function precomputeHAL() {
  ...
  let halIncidentsPath = locate("src/data/hal_incidents.csv") || locate("data/hal_incidents.csv");
  let halContractsPath = locate("data/hal-contracts-pbr.csv");
